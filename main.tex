...
conversão a tempo de execução.} O tipo de \texttt{x} é inferido como
\texttt{i32}, um número inteiro de 32 bits, com sinal.\footnote{Há uma
variedade de tipos escalares padrão, mas em geral, tipos numéricos têm tamanhos
explícitos: \texttt{u16} é um \textit{unsigned} de 16 bits, \texttt{u8} é um
\textit{byte} individual, \texttt{f64} é um \textit{float} de 64 bits,
\texttt{isize} é um \textit{signed} cujo tamanho é igual ao tamanho de uma
palavra na arquitetura atual, e assim por diante.}
